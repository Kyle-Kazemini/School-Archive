...
/**
 * This class represents a generic library.
 * 
...
public class LibraryGeneric<Type> {
	...
	public Type lookup(long isbn) {
		LibraryBookGeneric<Type> finalBook = null;
		
		for (LibraryBookGeneric<Type> book : library) {
			if (book.getIsbn() == isbn) {
				finalBook = book;
				return finalBook.getHolder();
			}
		}
		return null;
	}
	...
	public ArrayList<LibraryBookGeneric<Type>> getOverdueList(int month, int day, int year) {
		...
		GregorianCalendar input = new GregorianCalendar(year, month, day);
		
		for (LibraryBookGeneric<Type> book: library) {
			GregorianCalendar bookDate = book.getDueDate();
		    if (input.compareTo(bookDate) > 0) {
		    	overdue.add(book);
		    }
		}
		LibraryGeneric.sort(overdue, new OrderByDueDate());
		return overdue;
	}
	...
	public ArrayList<LibraryBookGeneric<Type>> getOrderedByTitle() {
		ArrayList<LibraryBookGeneric<Type>> titleList = new ArrayList<LibraryBookGeneric<Type>>();
		
		for (LibraryBookGeneric<Type> book: library)
		    titleList.add(book);
		    
		LibraryGeneric.sort(titleList, new OrderByTitle());
	...
	protected class OrderByDueDate implements Comparator<LibraryBookGeneric<Type>> {
		 ...
		 */
		public int compare(LibraryBookGeneric<Type> lhs, LibraryBookGeneric<Type> rhs) {
			return lhs.getDueDate().compareTo(rhs.getDueDate()) > 0 
					? 1 : (lhs.getDueDate().compareTo(rhs.getDueDate()) < 0 ? -1 : 0);
		}
	}
	...
	protected class OrderByTitle implements Comparator<LibraryBookGeneric<Type>> {
		 ...
		 */
		public int compare(LibraryBookGeneric<Type> lhs, LibraryBookGeneric<Type> rhs) {
			return lhs.getTitle().compareTo(rhs.getTitle()) > 0 
					? 1 : (lhs.getTitle().compareTo(rhs.getTitle()) < 0 ? -1 : 0);
		}
	}
}